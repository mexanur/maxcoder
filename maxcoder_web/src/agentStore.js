/**
 * Agent Store — manages projects, file tree, and agent chat state.
 * Separate from the main chat store to keep concerns clean.
 */
...
export const useAgentStore = create((set, get) => ({
  ...
  createProject: async (name) => {
    ...
    try {
      ...
      set({ activeProjectId: id, files: [], openFile: null })
      return id
    } catch (e) {
  ...
  deleteProject: async (id) => {
    try {
      ...
      if (get().activeProjectId === id) {
        set({ activeProjectId: null, files: [], openFile: null })
      }
    } catch (e) {
  ...
  setActiveProject: async (id) => {
    set({ activeProjectId: id, openFile: null })
  ...
  // ── Agent chat ────────────────────────────────────────────────────────────
  agentMessages:  [],   // [{id, role, content, done, ops?}]
  agentStreaming: false,
  lastOps:        [],   // last applied file ops from agent response
  ...
  sendAgentMessage: async (content, settings = {}) => {
    const {
      ...
      addAgentMessage, updateLastAgentAssistant,
      fetchFiles, files,
    } = get()
    ...
    if (!projectId) {
      projectId = await createProject('New Project')
    }
    ...
    try {
      ...
      while (true) {
        ...
        // Check for trailing ops event
        const eventIdx = full.indexOf('\n\n@@EVENT:')
        if (eventIdx !== -1) {
          const chatPart = full.slice(0, eventIdx)
          const eventStr = full.slice(eventIdx + 10)   // skip \n\n@@EVENT:
          updateLastAgentAssistant(chatPart, false)
          try {
            const event = JSON.parse(eventStr)
            if (event.event === 'ops') {
              set({ lastOps: event.applied, files: event.files || [] })
            }
          } catch (_) {}
        } else {
      ...
      // Final parse — strip event from displayed content
