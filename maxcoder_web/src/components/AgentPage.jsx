/**
 * AgentPage v2 — Phase 2 IDE
 *
 * ┌──────────────┬──────────────────────────────────────────────────────────┐
 * │ Projects     │  TOP TOOLBAR                                             │
 * │ sidebar      ├─────────────────────────┬────────────────────────────── │
 * │              │  File Tree              │  Editor / Diff / Preview tabs │
 * │              │                         │                               │
 * ├──────────────┤                         │                               │
 * │ (below)      │                         │                               │
 * └──────────────┴─────────────────────────┴───────────────────────────────┘
 *   Agent Chat panel slides up from bottom (toggle)
 */
...
const Icons = {
  ...
  save:    'M17 21H7a2 2 0 01-2-2V5a2 2 0 012-2h7l5 5v11a2 2 0 01-2 2z M17 21v-8H7v8 M7 3v5h8',
    ...
    <div className="flex flex-col h-full border-r border-border bg-surface w-44 flex-shrink-0">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-border">
        ...
        <button onClick={() => setNaming(v => !v)}
          className="p-0.5 rounded hover:bg-surface2 text-muted hover:text-text transition-colors">
      ...
      {naming && (
        <div className="p-2 border-b border-border">
          <input autoFocus value={name} onChange={e => setName(e.target.value)}
            ...
            className="w-full text-xs bg-surface2 border border-border rounded-lg px-2 py-1.5
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {projects.length === 0 && (
          <p className="text-[11px] text-muted text-center py-6 px-3 leading-relaxed">
...
function FileTree({ onOpen }) {
  ...
  if (!activeProjectId) return (
    <div className="flex items-center justify-center h-full text-muted text-xs text-center px-3">
      Select a project
    </div>
  )
      ...
      <div className="flex-1 overflow-y-auto py-1 font-mono">
        {files.length === 0 && (
          <p className="text-[11px] text-muted text-center py-8 px-3 leading-relaxed">
            No files yet.<br/><span className="text-accent">Ask the agent.</span>
          </p>
        )}
        {dirs.map(d => (
        ...
        {ffiles.map(f => (
          <div key={f.path}
            onClick={() => { openFileByPath(f.path); onOpen && onOpen(f.path) }}
...
// ═════════════════════════════════════════════════════════════════════════════
// Main Editor Area — Editor | Diff | Preview tabs
// ═════════════════════════════════════════════════════════════════════════════
function EditorArea() {
  const { openFile, saveFile, loadingFile, lastOps } = useAgentStore()
  const [tab,     setTab]     = useState('editor')  // editor | diff | preview
  const [draft,   setDraft]   = useState('')
  const [dirty,   setDirty]   = useState(false)
  const [prevContent, setPrev] = useState('')
  const [runResult, setRunResult] = useState(null)

  // When a new file is opened reset state
  useEffect(() => {
  ...
  // When agent edits THIS file, capture prev for diff
  useEffect(() => {
    if (!openFile || !lastOps?.length) return
    const op = lastOps.find(o => o.path === openFile.path && o.op === 'edit')
    if (op) {
      setPrev(draft)          // snapshot before applying
      setDraft(openFile.content)
      setDirty(false)
      setTab('diff')          // auto-switch to diff view
    }
    // Check for run result for this file's language
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setRunResult({ ok: false, stderr: String(e), stdout: '', lang: '' })
      setTab('preview')
    }
  }
  ...
  const openInVSCode = () => {
    if (!openFile) return
    // Works if vscode:// protocol is registered (standard VS Code install)
    window.open(`vscode://file/${encodeURIComponent(openFile.path)}`)
  }
  ...
  if (!openFile) return (
    <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 text-muted">
      <Ic name="code" className="w-10 h-10 opacity-20"/>
      <p className="text-sm">Select a file to edit</p>
      <p className="text-xs opacity-60">or ask the agent to create one</p>
    </div>
  )
      ...
      <div className="flex items-center gap-1 px-3 py-1.5 border-b border-border bg-surface flex-shrink-0 flex-wrap">
        ...
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            <Ic name={t.icon} className="w-3 h-3"/>
            {t.label}
          </button>
        ))}
        ...
        {(isRunnable || isVisual) && (
          <button onClick={handleRun}
            className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-green-500/15
                       border border-green-500/30 text-green-400 hover:bg-green-500/25 transition-colors">
            <Ic name="play" className="w-3 h-3"/> Run
          </button>
        )}
        {dirty && (
          <button onClick={() => handleSave()}
            className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-accent/20
                       border border-accent/30 text-accent hover:bg-accent/30 transition-colors">
            <Ic name="save" className="w-3 h-3"/> Save
          </button>
        )}
        <button onClick={openInVSCode}
          className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-surface2
                     border border-border text-muted hover:text-text transition-colors">
          <Ic name="vscode" className="w-3 h-3"/> Open in VS Code
        </button>
      </div>
        ...
        {tab === 'diff' && (
          prevContent
            ...
            : <div className="flex items-center justify-center h-full text-muted text-sm">
                No diff yet — diff appears after the agent edits this file.
              </div>
        )}
        {tab === 'preview' && (
...
function AgentChat({ collapsed, onToggle }) {
  ...
  return (
    <div className={`flex flex-col border-t border-border bg-surface transition-all duration-300
                     flex-shrink-0 ${collapsed ? 'h-10' : 'h-80'}`}>
      {/* Handle bar */}
      <div onClick={onToggle}
        className="flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-surface2 transition-colors select-none flex-shrink-0">
        <Ic name="robot" className="w-3.5 h-3.5 text-accent"/>
        <span className="text-xs font-semibold text-text">Agent Chat</span>
        {activeProjectId && (
          <span className="text-[10px] text-muted ml-1 font-mono truncate max-w-[120px]">
            — {activeProjectId.slice(0,8)}…
          </span>
        )}
        {agentStreaming && (
          <span className="ml-1 text-[10px] text-accent animate-pulse">● thinking</span>
        )}
                ...
                <div className="mt-3 flex flex-wrap gap-2 justify-center">
                  {[
                    ...
                    <button key={ex} onClick={() => sendAgentMessage(ex, settings)}
                      className="text-[11px] px-3 py-1.5 rounded-lg bg-surface2 border border-border
                                 hover:border-accent/50 text-muted hover:text-text transition-all">
                      {ex}
                    </button>
                  ))}
                </div>
              ...
              <div key={msg.id} className={`flex gap-2 ${msg.role==='user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-xl px-3 py-2 text-xs
                  ${msg.role==='user'
                    ...
                    : 'bg-surface2 border border-border rounded-tl-sm text-text'}`}>
                  {msg.role==='assistant'
          ...
          <div className="p-2 border-t border-border flex-shrink-0">
            <div className="flex gap-2 items-end">
              <textarea value={input} onChange={e => setInput(e.target.value)}
                ...
                rows={1}
                className="flex-1 bg-surface2 border border-border rounded-xl px-3 py-2 text-xs
                           text-text placeholder:text-muted focus:outline-none focus:border-accent resize-none"
              />
