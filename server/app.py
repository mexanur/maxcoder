...
async def chat(req: ChatReq):
    ...
    # FIX: pass mem_ctx and rag_ctx separately — no more double injection.
    # Web context is prepended directly to the user query.
...
async def chat_full(req: ChatReq):
    ...
    # FIX: same fix applied here — no double memory injection
    augmented_query = f"{web_ctx}\n\n{user_query}".strip() if web_ctx else user_query
    messages = build_prompt(augmented_query, history, mem_ctx, rag_ctx)
